 ...
 * system zone.
 */
export const zonedDate = (date: Date, timeZone: string): Date => {
  if (!timeZone) {
    return date;
  }
  try {
 ...
 */
export const invalidToken = (pattern: string, allowed: Set<string>): string | null => {
  const re = /%(-?)([A-Za-z%])/g;
 ...
 */
export const formatClock = (date: Date, format: string, locale: string): string => {
 ...
 */
export const formatCollapsedClock = (date: Date, twelveHour: boolean): string => {
  if (twelveHour) {
    const h12 = ((date.getHours() + 11) % 12) + 1;
 ...
 * the narrow strip.
 */
export const formatCollapsedDate = (date: Date): string => {
